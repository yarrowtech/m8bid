            ...
            <>
              <div className="mt-6 grid gap-4 md:grid-cols-3">
                <MetricCard
                  ...
                  sub={`Across ${analytics.totalCampaigns} campaigns`}
                />
                <MetricCard
                  title="Approved Campaigns"
                  value={analytics.approved}
                  sub={`${analytics.pending} campaigns still pending`}
                />
                <MetricCard
                  title="Funding Rate"
                  value={`${analytics.conversionRate}%`}
                  sub={`Against target ${formatCurrency(analytics.totalTarget)}`}
                />
              </div>
                          ...
                          <div className="flex h-52 w-full items-end">
                            <div
                              ...
                              style={{
                                height: `${Math.max(
                                  16,
                                  (item.total / analytics.maxMonthlyRaised) * 100
                                )}%`,
                              }}
                            />
                          </div>
                    ...
                    <div className="rounded-2xl bg-slate-50 p-4">
                      ...
                      <p className="mt-1 text-sm text-slate-500">
                        {analytics.bestCampaign
                          ? `${analytics.bestCampaign.projectTitle} has raised ${formatCurrency(
                              analytics.bestCampaign.raisedAmount
                            )}.`
                    ...
                    <div className="rounded-2xl bg-slate-50 p-4">
                      ...
                      <p className="mt-1 text-sm text-slate-500">
                        {analytics.topCategoryEntry
                          ? `${formatCategory(analytics.topCategoryEntry[0])} leads with ${
                              analytics.topCategoryEntry[1]
                            } campaign(s).`
