...
function CampaignRow({ item, deletingId, viewingId, onView, onDelete }) {
  ...
  const isViewing = viewingId === item?._id;

  return (
    <div className="grid min-w-[1180px] grid-cols-7 items-center gap-3 border-t border-slate-100 px-4 py-4 text-sm transition hover:bg-slate-50/70">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60"
        >
          <Eye size={14} />
...
export default function FundraiserCampaigns({ embedded = false }) {
  ...
  const handleViewCampaign = async (campaign) => {
    ...
    try {
      ...
      const campaignDetails = res?.data || res?.fundraiser || campaign;

      window.scrollTo({ top: 0, behavior: "smooth" });
      navigate(`/investment-detail/${id}`, {
        state: { campaign: campaignDetails },
      });
    } catch (err) {
      console.error("Failed to fetch campaign details:", err);
      alert(err?.message || "Failed to open campaign details");
    } finally {
      setViewingId("");
    }
  };
    ...
    <main className="flex-1 overflow-y-auto px-6 py-6">
      ...
      <div className="mt-6 rounded-[28px] border border-slate-100 bg-white shadow-sm">
        <div className="border-b border-slate-100 px-5 py-5">
          <h2 className="text-xl font-semibold text-slate-900">All Campaigns</h2>
          <p className="mt-1 text-sm text-slate-500">
            Open campaign details or remove campaigns from here.
          </p>
        </div>
